...
class TestTransform(unittest.TestCase):
    def test_translation(self):
        ...
        p = Point(-3, 4, 5)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(2, 1, 7)))

    def test_translation_inverse(self):
        transform = Transform().translation(5, -3, 2).inverse()
        p = Point(-3, 4, 5)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(-8, 7, 3)))

    def test_translation_vector(self):
        transform = Transform().translation(5, -3, 2)
        v = Vector(-3, 4, 5)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(v), v))

    def test_scaling_point(self):
        transform = Transform().scaling(2, 3, 4)
        p = Point(-4, 6, 8)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(-8, 18, 32)))

    def test_scaling_vector(self):
        transform = Transform().scaling(2, 3, 4)
        p = Vector(-4, 6, 8)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Vector(-8, 18, 32)))

    def test_scaling_inverse(self):
        transform = Transform().scaling(2, 3, 4).inverse()
        p = Vector(-4, 6, 8)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Vector(-2, 2, 2)))

    def test_scaling_reflection(self):
        transform = Transform().scaling(-1, 1, 1).inverse()
        p = Point(2, 3, 4)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(-2, 3, 4)))
    ...
    def test_rotation_x(self):
        ...
        self.assertTrue(
            tuple4_eq(half_quarter.matmul_tuple(p), Point(0, math.sqrt(2) / 2, math.sqrt(2) / 2))
        )
        self.assertTrue(tuple4_eq(full_quarter.matmul_tuple(p), Point(0, 0, 1)))
        self.assertTrue(
            tuple4_eq(
                half_quarter.inverse().matmul_tuple(p),
    ...
    def test_rotation_y(self):
        ...
        self.assertTrue(
            tuple4_eq(half_quarter.matmul_tuple(p), Point(math.sqrt(2) / 2, 0, math.sqrt(2) / 2))
        )
        self.assertTrue(tuple4_eq(full_quarter.matmul_tuple(p), Point(1, 0, 0)))
    ...
    def test_rotation_z(self):
        ...
        self.assertTrue(
            tuple4_eq(half_quarter.matmul_tuple(p), Point(-math.sqrt(2) / 2, math.sqrt(2) / 2, 0))
        )
        self.assertTrue(tuple4_eq(full_quarter.matmul_tuple(p), Point(-1, 0, 0)))
    ...
    def test_shearing(self):
        ...
        p = Point(2, 3, 4)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(5, 3, 4)))

        transform = Transform().shearing(0, 1, 0, 0, 0, 0)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(6, 3, 4)))

        transform = Transform().shearing(0, 0, 1, 0, 0, 0)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(2, 5, 4)))

        transform = Transform().shearing(0, 0, 0, 1, 0, 0)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(2, 7, 4)))

        transform = Transform().shearing(0, 0, 0, 0, 1, 0)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(2, 3, 6)))

        transform = Transform().shearing(0, 0, 0, 0, 0, 1)
        self.assertTrue(tuple4_eq(transform.matmul_tuple(p), Point(2, 3, 7)))
    ...
    def test_individual_chaining(self):
        ...
        p3 = B.matmul_tuple(p2)
        self.assertTrue(tuple4_eq(p3, Point(5, -5, 0)))

        p4 = C.matmul_tuple(p3)
    ...
    def test_group_chaining(self):
        ...
        B = Transform().scaling(5, 5, 5)
        C = Transform().translation(10, 5, 7)
        T = C.matmul(B.matmul(A))

        self.assertTrue(tuple4_eq(T.matmul_tuple(p), Point(15, 0, 7)))

        # pylint: disable=no-member
        T2 = Transform().rotation_x(math.pi / 2).scaling(5, 5, 5).translation(10, 5, 7)
        self.assertEqual(T, T2)
        self.assertTrue(tuple4_eq(T2.matmul_tuple(p), Point(15, 0, 7)))
